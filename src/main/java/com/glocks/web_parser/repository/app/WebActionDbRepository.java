...
//    List<WebActionDb> getListOfPendingTasks(@Param("feature")List<String> feature);
    ...
    @Transactional
//    @Query(value = "SELECT * FROM web_action_db u WHERE u.state NOT IN (4, 5) AND u.created_on <= SYSDATE - (1/24/60 * :min) AND u.feature IN (:feature) ORDER BY u.state, u.id ASC LIMIT 1", nativeQuery = true)
